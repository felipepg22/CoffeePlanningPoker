## Purpose
Define how users create, join, invite, recover, and stay synchronized in collaborative planning poker RoomSessions.
## Requirements
...
The system SHALL create a RoomSession only after a user submits a valid room name and participant display name for the create-room path, SHALL make the creator the initial facilitator Participant for that RoomSession, and SHALL initialize the planning poker session state from the backend snapshot.
...
- **THEN** the system enters the active named RoomSession with a stable room code, a shareable invite link, the creator represented as the facilitator Participant, a connected local room state, an empty Task list, no current Task, no active PlanningRound, no ParticipantVote state, and no saved FinalEstimate
...
The system SHALL allow a Participant to join a RoomSession using either a valid invite URL or a typed room code plus a valid participant display name, and SHALL populate the joined room from the backend-owned RoomSession snapshot.
...
- **THEN** the system enters the active RoomSession as that Participant and shows the current membership, Task list, current Task, PlanningRound state, ParticipantVote progress, revealed votes when applicable, saved FinalEstimates, and completed total when available
...
- **THEN** the system enters the matching RoomSession as that Participant and uses the same membership and planning poker session state as invite-link joins
...
The system SHALL synchronize active RoomSession membership, ParticipantPresence, and planning poker session state from ASP.NET Core SignalR room snapshots and membership events without requiring manual refresh.

#### Scenario: Backend owns shared room state
- **WHEN** two browser clients connect to the same RoomSession through the SignalR hub
- **THEN** both clients receive the same backend-owned RoomSession membership, Task, PlanningRound, ParticipantVote progress, reveal, and FinalEstimate state, subject to pre-reveal vote privacy rules

#### Scenario: Room snapshot is received
- **WHEN** the system receives a room snapshot for the active RoomSession
- **THEN** it replaces stale local membership and planning poker state with the snapshot Participants, ParticipantPresence states, Tasks, current Task, active PlanningRound, ParticipantVote visibility, reveal state, saved FinalEstimates, and completed total

#### Scenario: Participant joins the room
- **WHEN** a realtime event reports that a Participant joined the active RoomSession
- **THEN** the Participant appears in the room membership list and receives the current RoomSession planning poker state without requiring existing users to refresh
...
- **THEN** the system updates that ParticipantPresence while preserving the rest of the RoomSession state, including Tasks, current PlanningRound, ParticipantVote progress, revealed votes, and FinalEstimates

#### Scenario: Local connection state changes
- **WHEN** the current browser loses or restores its room connection
- **THEN** the system updates the local connection state separately from other Participants' ParticipantPresence states and does not treat local task, vote, or estimate state as authoritative

### Requirement: Refresh and Reconnect Preserve Recoverable Rooms
The system SHALL preserve only the minimum room recovery anchors needed to resume a recent RoomSession and SHALL recover membership and planning poker session state from backend-owned snapshots without treating locally stored room data as authoritative.
...
- **THEN** the system attempts to resume the RoomSession using the stored participant id and resume token, then replaces local state with the recovered room snapshot containing membership, Tasks, current Task, active PlanningRound, ParticipantVote visibility, reveal state, saved FinalEstimates, archived estimate total, and completed total when available
...
- **THEN** the system keeps the visible RoomSession, Participant list, Task list, current PlanningRound, vote progress, revealed values, and saved FinalEstimates from the last authoritative snapshot, shows a reconnecting local connection state, and attempts recovery without clearing the current user input
...
The system SHALL use stable participant identity for joins and resumes so repeated attempts for the same Participant and RoomSession do not create duplicate participant rows or duplicate ParticipantVote records when the room accepts the identity.
...
- **THEN** the system represents that user as one Participant, updates the existing ParticipantPresence, and preserves that Participant's current ParticipantVote for the active PlanningRound when one exists
...
The system SHALL show concise, actionable states for invalid invites, expired rooms, unavailable rooms, duplicate join failures, session command failures, and disconnected realtime connections.
...
- **THEN** the system communicates the disconnected or reconnecting state without hiding the invite details, current Participant list, Task list, current PlanningRound, vote progress, revealed votes, saved FinalEstimates, or completed total from the last authoritative snapshot

#### Scenario: Session command fails
- **WHEN** a Task, PlanningRound, ParticipantVote, reveal, reset, FinalEstimate, or completion command fails
- **THEN** the system keeps the user in the active RoomSession, preserves the latest authoritative snapshot, and shows a concise retryable error tied to the failed action
...
### Requirement: Room Snapshots Carry Active Planning State
The system SHALL include the active planning poker session state in RoomSession snapshots so create, join, refresh, reconnect, and duplicate-join flows recover from backend-owned state rather than local task or vote data.

#### Scenario: Active snapshot includes planning state
- **WHEN** the backend sends a RoomSession snapshot for an active room
- **THEN** the snapshot includes membership, ParticipantPresence, task list, current Task, active PlanningRound, pre-reveal ParticipantVote progress, local Participant vote visibility, revealed votes when applicable, saved FinalEstimates, archived estimate total, and completed total when available

#### Scenario: Local storage contains stale planning state
- **WHEN** a browser has stale locally stored Task, PlanningRound, ParticipantVote, or FinalEstimate data
- **THEN** the system ignores that local planning state and replaces visible room state with the backend-owned RoomSession snapshot

#### Scenario: Snapshot is sent before reveal
- **WHEN** a RoomSession snapshot is sent while the active PlanningRound is still voting
- **THEN** the snapshot exposes each Participant's voted state, exposes only the local Participant's own EstimateCard value to that Participant, and does not expose other hidden ParticipantVote values

#### Scenario: Snapshot is sent after reveal
- **WHEN** a RoomSession snapshot is sent after the active PlanningRound is revealed
- **THEN** the snapshot exposes revealed ParticipantVote values, the computed average estimate, and any saved FinalEstimate associated with the Task
